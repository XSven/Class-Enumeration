...
creates getters for custom attributes and as a final step prepares a list
of enum objects.
...
An enum class can be build at runtime too. In this case a direct call of the
C<import()> class method executes the build process. The first argument of
this method is an optional HASH reference. The C<class> key should be used
to set the enum class name. The C<import()> method returns this name.
